...
function Shop() {
   ...
   const handleShop=async ()=>{
      ...
      if (!result.ok) {
        console.error("get all item of shop data error", data.message);
        alert(data.message || "Failed to fetch order.");
        return;
      }

        console.log("this is data of get shop item if click",data);

      setItems(data.items)
      setShop(data.shop)
    
      alert("get all item of shop data ");
        ...
        <button className='absolute top-4 left-4 z-20 flex items-center gap-2 bg-black/50 hover:bg-black/70 text-white px-3 py-2 rounded-full shadow transition' onClick={()=>navigate("/")}>
         ...
         {shop && <div className='relative w-full h-64 md:h-80 lg:h-96'>
              ...
              <img src={shop?.image?.url} alt="" className='w-full h-full object-cover'/>
                ...
                <div className='absolute inset-0 bg-gradient-to-b from-black/40 to to-black 20 flex flex-col justify-center items-center text-center px-4'>
